...
for LANGUAGE in "de" ; do
  ...
  magick "$SRCPATH/cards/back_dcm.png" \
  ...
  for file in src/cards/$LANGUAGE/[0-9][0-9].md; do
    ...
    if [ "$SIZE" -gt "164" ] ; then
      magick "$SRCPATH/cards/front_dcm.png" \
      -pointsize 50 -fill white -font "ext/static/OpenSans-SemiBold.ttf" -draw "text 222,275 '$TITLE'" \
      -pointsize 30 -font "ext/static/OpenSans-Regular.ttf" -draw "text 1132,285 '$cardnumber'" \
      -pointsize 164 -gravity west -fill black -size 930x440 label:"$TEXT" -geometry +212+55\
      -composite "$BUILDPATH/$LANGUAGE/images/cards/$cardnumber.png"
    else
      magick "$SRCPATH/cards/front_dcm.png" \
      -pointsize 50 -fill white -font "ext/static/OpenSans-SemiBold.ttf" -draw "text 222,275 '$TITLE'" \
      -pointsize 30 -font "ext/static/OpenSans-Regular.ttf" -draw "text 1132,285 '$cardnumber'" \
      +pointsize -gravity west -fill black -size 930x440 label:"$TEXT" -geometry +212+55\
      -composite "$BUILDPATH/$LANGUAGE/images/cards/$cardnumber.png"
    fi
    let cardcount++
    echo -n .
  ...
  for i in $(seq 1 $ADDITIONALCARDS); do
    magick "$SRCPATH/cards/front_dcm.png" \
    -pointsize 50 -fill white -font "ext/static/OpenSans-SemiBold.ttf" -draw "text 222,275 '$TITLE'" \
    -pointsize 20 -gravity northeast -font "ext/static/OpenSans-Regular.ttf" -draw "text 322,265 '$INDIVIDUELL'" \
